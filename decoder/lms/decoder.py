...
class LmsDecoder:
    ...
    def decoderHandler(self, startRunning:bool):
        ...
        if (self.decoderHandlerRunning):
            self.log.warning("Selcall Decoder process is already running...")
            ...
            while (self.decoderHandlerRunning):
                ...
                if foundPhasing:
                    try:
                        ...
                        # TODO: Complete for Selcall Specific
                        # # Decode Message
...
def main():
    ...
    print(f"DEBUG: Remaining strYBY - Len: [{dec.bits.length()}] - Data: [{"".join(dec.bits.bits)}]")
